...
export default function Home() {
  return (
    <div className="grid min-h-screen items-center justify-center px-4 lg:px-24 ">
      <Hero />
      <div className="">
        <p className="text-3xl lg:text-5xl font-semibold pt-4">📰 Featured Posts</p>
        <div className="my-8 grid lg:flex lg:flex-row gap-x-8 gap-y-8">
          {featuredPosts.map((post, idx) => (
            <FeaturedPost key={idx} title={post.title} date={post.date} link={post.link} />
          ))}
        </div>
        <p className="text-3xl lg:text-5xl font-semibold pt-4">📑 Spotlight Publications</p>
        <div className="my-8 grid space-y-4">
          {pubs.map((article, idx) => (
            <SpotlightArticle key={idx} {...article} />
          ))}
        </div>
        <p className="text-3xl lg:text-5xl font-semibold pt-4">🎥 Video Content</p>
